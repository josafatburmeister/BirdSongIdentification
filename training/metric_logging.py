...
class TrainingLogger:
    ...
    @staticmethod
    def log_metrics_in_kubeflow(best_average_metrics: Metrics, best_minimum_metrics: Metrics) -> None:
        """
        Logs metrics
    ...
    @staticmethod
    def store_summary_metrics(metrics: Metrics) -> None:
        """
        ...
        wandb.run.summary[f"mean_f1_score_avg_model"] = torch.mean(
            metrics.f1_score())
        wandb.run.summary[f"min_f1_score_avg_model"] = torch.min(
            metrics.f1_score())
    ...
    def __init__(self, model_trainer, config: Dict[str, Any] = None, is_pipeline_run: bool = False,
        ...
        if track_metrics:
            ...
            wandb.init(project=wandb_project_name,
                       entity=wandb_entity_name, config=config)
    ...
    def __print_metrics(self, metrics_object: Metrics) -> None:
        ...
        logger.info(tabulate(metric_rows, headers=table_headers,
            ...
            for name, get_method in self.metrics.items():
                ...
                if metric.shape.numel() == 1:
                    ...
                    for class_id in range(metric.shape.numel()):
                        metric_name = self.id_to_class_mapping[class_id] + \
                            '_' + name + '_' + phase
    ...
    def print_model_summary(self, best_average_epoch: int, best_average_metrics: Metrics, best_minimum_epoch: int,
        ...
        logger.info('-' * 10)
        logger.info(
            "Validation metrics of model with best average F1-Scores (epoch %s):", best_average_epoch)
        self.__print_metrics(best_average_metrics)
        logger.info(
            "Validation metrics of model with best minimum F1-Score (epoch %s):", best_minimum_epoch)
