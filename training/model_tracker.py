...
class ModelTracker:
    def __init__(self, pathmanager: filepaths.PathManager, experiment_name: str, id_to_class_mapping: dict, is_pipeline_run: bool, multi_label_classification: bool = True):
        ...
        self.best_average_metrics = metrics.Metrics(num_classes=self.num_classes, multi_label=self.multi_label_classification)
        self.best_average_model = None
        ...
        for class_name in self.id_to_class_mapping.values():
            ...
            self.best_metrics_per_class[class_name] = metrics.Metrics(num_classes=self.num_classes,
                                                                      multi_label=self.multi_label_classification)
            self.best_models_per_class[class_name] = None

    def track_best_model(self, model, model_metrics: metrics.Metrics, epoch: int):
        ...
        if self.multi_label_classification:
            for class_id, class_name in self.id_to_class_mapping.items():
                if self.best_metrics_per_class[class_name].f1_score()[class_id].item() < model_metrics.f1_score()[class_id].item() \
                        or self.best_metrics_per_class[class_name].f1_score()[class_id].item() == model_metrics.f1_score()[
                    class_id].item() \
    ...
    def save_best_models(self, run_id: str):
        ...
        for model, model_name in [(self.best_average_model, f"{self.experiment_name}_avg_model_{current_time_str}.pt"), (self.best_minimum_model, f"{self.experiment_name}_min_model_{current_time_str}.pt")]:
        ...
        if self.multi_label_classification:
            for class_name, model in self.best_models_per_class.items():
                model_path = os.path.join(self.path.model_dir(), self.experiment_name, f"{self.experiment_name}_{class_name}_{current_time_str}.pt")
                if run_id:
                    model_path.replace(".pt", f"_{run_id}.pt")
                self.save_model(model, model_path)
